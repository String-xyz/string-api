...
func TestEncryptDecryptString(t *testing.T) {
	...
	strEncrypted, err := common.EncryptString(str, "secret_encryption_key_0123456789")
	assert.NoError(t, err)

	strDecrypted, err := common.DecryptString(strEncrypted, "secret_encryption_key_0123456789")
	assert.NoError(t, err)
...
func TestEncryptDecryptObject(t *testing.T) {
	...
	objDecrypted, err := common.DecryptString(objEncrypted, "secret_encryption_key_0123456789")
	assert.NoError(t, err)
...
func TestEncryptDecryptUnencoded(t *testing.T) {
	...
	objDecrypted, err := common.Decrypt[randomObject1](objEncrypted, "secret_encryption_key_0123456789")
	assert.NoError(t, err)
