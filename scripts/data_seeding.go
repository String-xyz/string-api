...
func main() {
	...
	// String User, pointless
	userString, err := repos.User.Create(model.User{Type: "Developer", Status: "Developing", FirstName: "Hermes", MiddleName: "Thoth", LastName: "Trismegistus"})
	if err != nil {
		panic(err)
	}
	...
	_, err = repos.Instrument.Create(model.Instrument{Type: "Credit Card", Status: "Ephemeral", Network: "Mastercard", Last4: "4242", UserID: userString.ID})
	if err != nil {
		panic(err)
	}
	/*instrumentDeveloperWallet*/ _, err = repos.Instrument.Create(model.Instrument{Type: "Crypto Wallet", Status: "Ephemeral", Network: "Ethereum", PublicKey: stringPublicAddress, Last4: "4242", UserID: userString.ID})
	if err != nil {
		panic(err)
	}
