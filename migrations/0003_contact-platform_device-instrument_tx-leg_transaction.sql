...
CREATE TABLE transaction (
  ...
  created_at TIMESTAMP WITH TIME ZONE NOT NULL, -- time transaction entry was initially created
  ...
  network_fee BIGINT DEFAULT 0, -- The true amount of gas in wei that was used to facilitate the transaction
  contract_params JSONB DEFAULT '{}'::JSONB, -- The parameters passed into the EVM function call passed into the endpoint
  contract_func JSONB DEFAULT '{}'::JSONB, -- The declaration of the EVM function call passed into the endpoint
  transaction_amount BIGINT DEFAULT 0, -- The cost in native token wei of the transaction passed into the endpoint
  ...
  processing_fee BIGINT DEFAULT 0, -- CC (ie checkout) processing fee in (USD/???) wei
  processing_fee_asset UUID REFERENCES asset (id), -- CC processing fee asset id in asset table (ie id for USD)
  string_fee BIGINT DEFAULT 0 -- Amount in (USD!) wei that we charged to facilitate this transaction, likely always will be USD
);
